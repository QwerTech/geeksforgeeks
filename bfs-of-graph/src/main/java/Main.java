...
class Traversal {
  ...
  static void bfs(int s, ArrayList<ArrayList<Integer>> list, boolean vis[], int nov) {
    ...
    visit(Collections.singletonList(root));
  }


  private static void visit(List<Graph.Node> nodes) {
    if (nodes.isEmpty()) {
      return;
    }
    nodes.forEach(Traversal::print);
    visit(nodes.stream().flatMap(n -> n.to.stream()).filter(n -> !n.visited)
